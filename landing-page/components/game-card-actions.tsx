...
export function GameCardActions({ game, profileUsername, showRemix = false, onDelete }: GameCardActionsProps) {
  ...
  const showViewButton = (game.status === "published" || game.status === "built") && game.bundle_url;
  const showMakePublicButton = (game.status === "published" || game.status === "built") && game.visibility === "private";
