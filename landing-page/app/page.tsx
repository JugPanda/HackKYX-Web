...
const stats = [
  { label: "Zombie worlds alive", value: "240+" },
  { label: "Custom survivors", value: "6,500" },
  { label: "Preview builds", value: "Unlimited" },
];

const workflow = [
  {
    title: "Branch outbreaks safely",
    description: "Spin up novel zombie encounters on feature branches. Share URLs that auto-refresh as you tweak rooms.",
    icon: <Layers className="h-5 w-5 text-emerald-300" />,
  },
  {
    title: "Review like pull requests",
    description: "Inline comments on survivor bios, diffs on infection curves, and approvals that gate deploys.",
    icon: <Users className="h-5 w-5 text-sky-300" />,
  },
  {
    title: "Automate publish steps",
    description: "GitHub Actions-ready scripts export pygbag builds and attach playable zombie sims to releases.",
...
const engineHighlights = [
  {
    ...
    body: "Seamless transitions across safehouses, tunnels, and rooftops with persistent state and dynamic horde logic.",
    icon: <Sparkles className="h-5 w-5 text-emerald-300" />,
  },
  {
    title: "Programmable undead",
    body: "Write logic in JSON or drop-in Python behaviors. Mods hot-reload while hordes keep shambling.",
    icon: <Braces className="h-5 w-5 text-sky-300" />,
  },
  {
    title: "Instant web export",
    body: "Bundle to web with pygbag compatibility and deploy a zombie scenario directly to Pages or itch.io.",
    icon: <Zap className="h-5 w-5 text-fuchsia-300" />,
  },
  {
    title: "Secure sync",
    body: "Deterministic save slots + replay data so squads coordinate cures without desync.",
...
const roadmap = [
  {
    phase: "Now",
    title: "Multi-room outbreak",
    body: "Link dozens of handcrafted safehouses, alleys, and rooftops with portals, parallax, and persistent supplies.",
  },
  {
    phase: "Next",
    title: "Madlib mod editor",
    body: "Live-preview JSON mods with a guided UI that swaps names, lore, art, and difficulty without touching files.",
  },
  {
    phase: "Later",
    title: "Creator cloud saves",
    body: "Zero-config spaces for studios with per-branch builds, user-generated outbreak data, and review apps.",
  },
];

const testimonials = [
  {
    quote: "We reskin the zombie story for every hackathon without touching JSON. The Madlib UI keeps players creating.",
    author: "Malik Ortega",
    role: "Game Design Lead",
  },
  {
    quote: "KYX let our club ship a zombie survival sim in a weekend. The workflow matches how we already ship code.",
    author: "Nova Bennett",
...
const creativeFeatures = [
  ...
  },
  {
    ...
    description: "Upload art, drag modules, and preview the outbreak card exactly how players will see it.",
  },
  {
...
const onboardingSteps = [
  {
    ...
    description: "Name your survivor, nemesis, and safehouse in plain language—no config files or syntax worries.",
  },
  {
    title: "Add vibes & art",
    description: "Choose tone, difficulty, and drop custom art so the card feels like your squad’s inside joke.",
  },
  {
    title: "Share the outbreak",
...
const madlibFields = [
  ...
  },
  {
    ...
    helper: "Describe the infected faction hunting the player.",
  },
  {
    label: "Safehouse photo",
    placeholder: "Drop PNG or JPG",
    helper: "Drag-and-drop art; KYX optimizes it for web builds.",
  },
  {
    label: "Victory condition",
    placeholder: "Synthesize cure at Riverside Lab",
        ...
        <main className="mx-auto w-full max-w-6xl flex-1 px-6 pb-24">
          <section id="hero" className="grid gap-12 py-12 lg:grid-cols-[1.1fr_0.9fr]">
            <div className="space-y-8">
              <Badge variant="success" className="w-fit border-emerald-500/50 bg-emerald-500/10 text-emerald-200">
                KYX · Zombie sandbox meets GitHub workflow
              </Badge>
              <div className="space-y-6">
                <h1 className="text-4xl font-semibold leading-tight text-white sm:text-5xl lg:text-[3.5rem]">
                  Let’s build the last safe engine on earth—together.
                </h1>
                <p className="max-w-2xl text-lg text-slate-300">
                  KYX is a zombie-inspired platformer built with Python and tuned for instant web deploys.
                  Non-technical gamers branch outbreak scenarios, remix JSON-driven rooms through Madlib
                  prompts, and share playable browser builds without touching a console.
                </p>
                <div className="rounded-2xl border border-slate-800/70 bg-slate-950/40 p-5 text-sm text-slate-300">
                  ...
                  <ol className="mt-3 space-y-2">
                    <li>
                      <span className="font-semibold text-white">1. Name your heroes.</span> Use Madlib prompts to
                      lock in survivors, codenames, and nemesis lore.
                    </li>
                    <li>
                      <span className="font-semibold text-white">2. Paint the safehouse.</span> Drop images and describe
                      the vibe in everyday language.
                    </li>
                    <li>
                      <span className="font-semibold text-white">3. Ship a link.</span> Our Python + Pygame engine turns
                      everything into JSON and spins a browser-ready build for your friends.
                    </li>
                  </ol>
                </div>
              </div>
            ...
            >
              <div className="absolute inset-x-6 top-6 flex items-center justify-between text-xs text-slate-400">
                <span>main • kyx-engine</span>
                <span>Auto deploy</span>
              </div>
              <CardContent className="mt-12 space-y-6">
                <div className="rounded-2xl border border-slate-800/80 bg-slate-950/50 p-4">
                  ...
                  <div className="mt-4 space-y-4">
                    {[
                      { title: "Atrium", status: "Synced" },
                      { title: "Observatory", status: "Awaiting review" },
                      { title: "Foundry", status: "Preview build" },
                    ].map((room) => (
                      <div key={room.title} className="flex items-center justify-between rounded-xl border border-slate-800/70 bg-slate-900/60 px-4 py-3">
                        <div>
                          <p className="font-medium text-white">{room.title}</p>
                          <p className="text-xs text-slate-400">Dynamic lighting, parallax</p>
                        </div>
                        <Badge className="border-slate-700 bg-slate-950/60 text-xs text-slate-200">
                          {room.status}
                        </Badge>
                      </div>
                    ))}
                ...
                <div className="rounded-2xl border border-emerald-500/40 bg-emerald-500/10 p-4 text-sm text-emerald-100">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs uppercase tracking-[0.35em] text-emerald-200/80">CI Pipeline</p>
                      <p className="text-lg font-semibold">Deploy to pages</p>
                    </div>
                    <Cpu className="h-5 w-5" />
                  </div>
                  <p className="mt-2 text-emerald-100/80">
                    Every merge spins a fresh pygbag export and posts a playable link on the pull request.
                  </p>
                </div>
              </CardContent>
            </Card>
          ...
          >
            <div className="space-y-6">
              ...
              <h2 className="text-3xl font-semibold text-white sm:text-4xl">
                Customize outbreaks without touching JSON.
              </h2>
              <p className="text-slate-400">
                Players type in names, upload survivor photos, and remix lore in plain
                language. KYX turns those prompts into JSON templates that patch the
                running game, so every squad ships a personal zombie story in minutes.
              </p>
            ...
            <Card className="border-slate-800/70 bg-[#0d1117] shadow-[0_25px_70px_rgba(4,6,11,0.8)]">
              <CardContent className="space-y-6 p-8">
                <div>
                  ...
                  <h3 className="text-lg font-semibold text-white">Safehouse setup</h3>
                </div>
                ...
                <Button className="w-full">Generate outbreak JSON</Button>
              </CardContent>
            </Card>
          </section>
