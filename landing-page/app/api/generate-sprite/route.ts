...
// Alternative: Using a simpler DALL-E 2 model for faster/cheaper generation
export async function POST_DALLE2(request: NextRequest) {
  try {
    const body = await request.json();
    const { description, type, style } = body;
    ...
    const detailedPrompt = `${artStyle} game sprite, ${description}, ${spriteType}, transparent background, video game asset, 32x32 pixel style`;
